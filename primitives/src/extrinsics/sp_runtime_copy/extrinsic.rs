...
use crate::extrinsics::sp_runtime_copy::{
	traits::{
		transaction_extension::TransactionExtension, ExtrinsicCall, ExtrinsicLike,
		ExtrinsicMetadata, SignaturePayload,
	},
...
impl<Address, Call, Signature, Extension: TransactionExtension<Call>> ExtrinsicMetadata
	for UncheckedExtrinsic<Address, Call, Signature, Extension>
{
	...
	Extension: Decode,
{
	fn decode<I: Input>(input: &mut I) -> Result<Self, codec::Error> {
		...
		let function = Call::decode(input)?;
...
/// actually contains.
pub struct SignedPayload<Call, Extension: TransactionExtension<Call>>(
