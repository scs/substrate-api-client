...
use substrate_api_client::{
	ac_primitives::{Config, DefaultAssetConfig, ExtrinsicSigner},
	rpc::JsonrpseeClient,
	Api, GetAccountInformation, GetStorage,
};

// To test this example in CI, we run it against the Substrate kitchensink node. Therefore, we use the DefaultAssetConfig
// ! Careful: Most runtimes uses plain as tips, they need a polkadot config.

type AccountInfo = GenericAccountInfo<
	<DefaultAssetConfig as Config>::Index,
	<DefaultAssetConfig as Config>::AccountData,
>;

type Balance = <DefaultAssetConfig as Config>::Balance;
