...
impl KeystoreInner {
    ...
    fn insert_ephemeral_pair<Pair: PairT>(
        &mut self,
        pair: &Pair,
        seed: &str,
        key_type: KeyTypeId,
    ...
    fn key_pair_by_type<Pair: PairT>(
        ...
        let phrase =
            if let Some(p) = self.key_phrase_by_type(public.as_slice(), key_type)? {
                p
            } else {
                return Ok(None)
            };

        let pair = Pair::from_string(&phrase, self.password())
...
mod tests {
    ...
    impl KeystoreInner {
        fn insert_ephemeral_from_seed<Pair: AppPair>(
            &mut self,
            seed: &str,
        ) -> Result<Pair> {
    ...
    fn store_unknown_and_extract_it() {
        ...
        let key_pair =
            sr25519::AppPair::from_string(secret_uri, None).expect("Generates key pair");
    ...
    fn generate_with_seed_is_not_stored() {
        ...
        let store = LocalKeystore::open(temp_dir.path(), None).unwrap();
        let _alice_tmp_key =
            SyncCryptoStore::sr25519_generate_new(&store, TEST_KEY_TYPE, Some("//Alice"))
                .unwrap();
