...
fn main() {
	...
	let api = Api::new(client).map(|api| api.set_signer(signer.clone())).unwrap();
	...
	let tx_hash = api.send_extrinsic(xt.hex_encode(), XtStatus::Finalized).unwrap().unwrap();
	println!("[+] Transaction got finalized. Hash: {:?}\n", tx_hash);

	// get the index at which Alice's Kitty resides. Alternatively, we could listen to the
	// StoredKitty event similar to what we do in the example_contract.
	let index: u64 = api
