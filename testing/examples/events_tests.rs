...
type Hash = <DefaultAssetConfig as Config>::Hash;
...
async fn main() {
	...
	api.set_signer(ExtrinsicSigner::<DefaultAssetConfig>::new(alice_pair));
	...
	for _ in 0..5 {
		let event_records = event_subscription
			.next_events::<RuntimeEvent, <DefaultAssetConfig as Config>::Hash>()
			.unwrap()
			.unwrap();
