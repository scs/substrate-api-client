    ...
    Client: RpcClient,
{
    ...
    pub fn get_account_info(
        ...
        let storagekey: sp_core::storage::StorageKey =
            self.metadata.storage_map_key::<AccountId, AccountInfo>(
                "System",
                "Account",
                address.clone(),
            )?;
    ...
    pub fn get_storage_map<K: Encode, V: Decode + Clone>(
        ...
        let storagekey = self.metadata.storage_map_key::<K, V>(
            storage_prefix,
            storage_key_name,
            map_key,
        )?;
...
pub enum ApiClientError {
    ...
    Genesis,
    #[error(
        "Fetching runtime version failed. Are you connected to the correct endpoint?"
    )]
    RuntimeVersion,
