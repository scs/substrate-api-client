...
use crate::{
    rpc::ws_client::on_extrinsic_msg_submit_only,
    std::{
        rpc::{
            json_req,
            ws_client::{
...
impl RpcClientTrait for WsRpcClient {
    ...
    fn send_extrinsic(
        ...
        let jsonreq = match exit_on {
            XtStatus::SubmitOnly => {
                json_req::author_submit_extrinsic(&xthex_prefixed).to_string()
            }
...
impl WsRpcClient {
    ...
    fn start_rpc_client_thread(
        ...
        let _client = thread::Builder::new().name("client".to_owned()).spawn(
            move || -> WsResult<()> {
                connect(url, |out| {
                    RpcClient {
                        out,
