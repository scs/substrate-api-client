...
pub type OnMessageFn =
    fn(msg: Message, out: Sender, result: ThreadOut<String>) -> WsResult<()>;
    ...
    Client: RpcClientTrait + Subscriber,
{
    ...
    pub fn wait_for_raw_event(
        ...
        loop {
            ...
            let _events =
                event_decoder.decode_events(&mut Vec::from_hex(event_str)?.as_slice());
...
pub fn on_subscription_msg(
    ...
    let value: serde_json::Value = serde_json::from_str(msg.as_text()?)
        .map_err(|e| Box::new(RpcClientError::Serde(e)))?;
...
mod tests {
    ...
    fn result_from_json_response_errs_on_error_response() {
        let _err_raw = r#"{"code":-32602,"message":"Invalid params: invalid hex character: h, at 284."}"#;
